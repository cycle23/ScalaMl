...
object DisplayUtils {
  ...
  // Default return value after info display
  private val DEFAULT_ERROR_RETURN = -1
  // Default return value for error
  private val DEST_CONSOLE = 0x01
  // Flag to dump computation results on std out
  private val DEST_LOGGER = 0x02
  // Flag to dump computation results into log4j log
  ...
  implicit class extendTry[T](_try: scala.util.Try[T]) {
    ...
    def error[U >: T](default: => U, comment: String, logger: Logger): U =
      _try.getOrElse({
        processError(comment, logger); default
      })
  }
