...
/**
  * '''Purpose:'''Singleton to evaluate the binary (2 class) multi-layer perceptron with
  ...
  *      Test case
  */
object BinaryDeepMLPEval extends Eval {
  ...
  override protected def run(args: Array[String]): Int = {
    ...
    // Test the impact of the learning rate eta on the convergence of the gradient descent
    val etaValues = List[Double](0.01, 0.02, 0.03, 0.1)
    val data = etaValues.flatMap(testEta(_, xt, yt, hidden)).map { case (x, s) => (x.toVector, s) }
    val legend = new Legend("Err", "MLP [2-7-3-1] training - learning rate", "Epochs", "Error")

    LinePlot.display(data, legend, new LightPlotTheme)

    // Test the impact of the momentum rate, alpha on the convergence of the gradient descent
    val alphaValues = List[Double](0.0, 0.2, 0.4, 0.6)
    val data2 = alphaValues.flatMap(testAlpha(_, xt, yt, hidden)).map { case (x, s) => (x.toVector, s) }
  ...
  private def testEta(
                       ...
                       xt: XVSeries[Double],
                       yt: XVSeries[Double],
                       hidden: Array[Int]): Option[(mutable.ArrayBuffer[Double], String)] = {
